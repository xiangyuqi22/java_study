...
------------------------------------------------------------------------------*/
	...
	static 
	{
		...
		try 
		{	
			System.err.println("RXTXDriver = 107");
			CommDriver RXTXDriver = (CommDriver) Class.forName("com.cattsoft.rxtxcomm.RXTXCommDriver").newInstance();
			System.err.println("RXTXDriver = " + RXTXDriver );
			RXTXDriver.initialize();
		} 
		catch (Throwable e) 
		{
			System.err.println(e + " thrown while loading " + "com.cattsoft.rxtxcomm.RXTXCommDriver");
		}
	...
	{ 
		...
		synchronized(Sync) {
			...
			try 
			{
				...
				//{@link #addPortName(java.lang.String, int, com.cattsoft.rxtxcomm.CommDriver)}
...
------------------------------------------------------------------------------*/
	...
	public CommPort open(String TheOwner, int i) 
		throws com.cattsoft.rxtxcomm.PortInUseException
	{ 
		...
		if (!isAvailable)
		{
			throw new com.cattsoft.rxtxcomm.PortInUseException(getCurrentOwner());
		}
		...
		try {
			...
			else
			{
				...
				throw new com.cattsoft.rxtxcomm.PortInUseException( err_msg );
			}
		} finally {
			if(commport == null) {
